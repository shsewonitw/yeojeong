...
function wsConnect(user_id){
	...
	wsocket = new WebSocket("ws://192.168.0.87:8080/yeojeong/wsa")
		...
		var d = new Date();
   		...
   		if( $("#a_"+sender_id).length  == 0 ){
   			...
   			$("#"+sender_id+"_messageWindow").html($("#"+sender_id+"_messageWindow").html()
   					+ '<div class="alert alert-info" role="alert" style="clear:both;float:left;max-width:280px;word-break:break-all;">user_'+sender_id+'님이 '+d+' 에 접속하였습니다.</div>');
...
function sendMessage(user_id){
	...
	if(inputVal == ""){
		return;
		 ...
		 $(".all_messageWindow").append("<div class='alert alert-danger' role='alert' style='clear:both;float:right;max-width:280px;word-break:break-all;text-align:right;'>-전체메세지-<br>" + inputVal.substr(5) + "</div><br/>");
	}
	else {
