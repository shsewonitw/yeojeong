...
<form action="<%=request.getContextPath()%>/transform" method="post" enctype="multipart/form-data">
	...
	<select class="form-control" id="id_travel" name="data" style="width:350px">
	...
	<c:forEach items="${travelRegistList}" var="travelList" varStatus="status">
			<option   
				<c:forEach items="${withme_viewList }" var="withmeview">
					<c:if test="${travelList.travel_id eq withmeview.travel_id }">disabled="disabled"
					</c:if>
				</c:forEach>
	...
	<select class="form-control" id="id_age" name="category_age" style="width:200px">
	...
	<select class="form-control" id="id_style" name="category_style" style="width:200px">
	  ...
	  <option value="5">엑티비티</option>
		...
		<div class="write_btn">
			<input class="btn btn-primary" id="id_submit" type="submit" value="작성">
		</div>
	<div class="bottom">
